...
itpr = os.path.join(venv, 'Scripts', 'python.exe')
...
# Upgrade [pip]
print("Upgrading [pip]")
subprocess.check_call(itpr + ' -m pip install --upgrade pip')
# Upgrade [setuptools]
print("\nUpgrading [setuptools]")
subprocess.check_call(itpr + ' -m pip install --upgrade setuptools')

# Install [NumPy]
print("\nInstalling [NumPy]")
subprocess.check_call(itpr + ' -m pip install "numpy>=2.0.0"')
# Install [SciPy]
print("\nInstalling [SciPy]")
subprocess.check_call(itpr + ' -m pip install "scipy>=1.14.0"')
# Install [h5py] for HDF5 files
print("\nInstalling [h5py]")
subprocess.check_call(itpr + ' -m pip install h5py')
# Install [matplotlib] for general plotting
print("\nInstalling [matplotlib]")
subprocess.check_call(itpr + ' -m pip install "matplotlib>=3.8.4"')
# Install [plotext] for CLI plotting
print("\nInstalling [plotext]")
subprocess.check_call(itpr + ' -m pip install plotext')
# Install [PySide6] for GUI
print("\nInstalling [PySide6]")
subprocess.check_call(itpr + ' -m pip install "PySide6>=6.8"')
# Install [PyQtGraph] for real-time plotting
print("\nInstalling [PyQtGraph]")
subprocess.check_call(itpr + ' -m pip install pyqtgraph')
# Install [PyTorch]
print("\nInstalling [PyTorch]")
try:
    for line in subprocess.check_output('nvcc --version').decode().split('\n'):
        if 'release' in line:
            ...
            subprocess.check_call(itpr + command)
            break
