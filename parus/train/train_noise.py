...
class LSTM(nn.Module):
    def __init__(self, n_input, n_hidden, n_layers, dropout):
        super(LSTM, self).__init__()
        self.lstm = nn.LSTM(n_input, n_hidden, n_layers, dropout=dropout, bidirectional=True)
        self.out_layer = nn.Linear(2*n_hidden, 1)
        self.n_layers = n_layers
        self.n_hidden = n_hidden

    def forward(self, input, hidden):
        lstm_output, hidden = self.lstm(input, hidden)
        output = self.out_layer(lstm_output)
        return output, hidden

    def init_hidden(self, batch_size):
        weight = next(self.parameters())
        return (weight.new_zeros(self.n_layers*2, SEQ_LEN, self.n_hidden),
                weight.new_zeros(self.n_layers*2, SEQ_LEN, self.n_hidden))
